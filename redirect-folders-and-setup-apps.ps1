# Setup an Azue Active Directory-joined Windows 10 Machine
#
# Tasks:
# - Enable Modern Auth on Office 2013
# - Enable Modern Auth in OneDrive
# - Force OneDrive for Business (no personal sync)
# - Redirect folders to OneDrive [Optional]
# - ClientBase Registry Settings [You probably don't need these]
# - Sabre/ClientBase Folder Permissions [You probably don't need these]
#
#
# Some parts derived from http://stackoverflow.com/questions/25709398/set-location-of-special-folders-with-powershell
# 
# Copyright (c) 2017 Nick Semenkovich <semenko@alum.mit.edu>.
...
$markerpath = "$env:USERPROFILE\aad-sync-done-v1.txt"

########################################################
## AAD Join Sanity Testing (Don't Run Elsewhere)
########################################################

# Test that we're on Azure Active Directory
[string]$upn = (whoami /upn) 2>&1


# Make noise if we're run outside of AAD or @example.com domain.
# The invocation .bat script should've tested this already, hence the noise.
if ($upn -like "*not a domain user*") {
    [System.Windows.MessageBox]::Show('Not on Azure Active Directory. Do not run this script.')
    exit
} elseif (-not ($upn -like "*$domain")) {
    [System.Windows.MessageBox]::Show('Incorrect AAD domain. Did you set the parameters correctly?')
    exit
} elseif ($upn -like "*$domain") {
    # Hooray! We'll continue.
} else {
    # Not sure how to get here, but I'm somewhat suspicious of Powershell, so we're pretty defensive.
    [System.Windows.MessageBox]::Show('Unexpected AAD script error.')
    exit
}
...
########################################################
## Folder Redirection (Local Paths --> OneDrive)
########################################################
...
'@
    ...
    ## Move and delete files (be careful here!)
	# $Leaf = Split-Path -Path "$Path" -Leaf
	# Move-Item "$HOME\$Leaf\*" $Path
	# rd $HOME\$Leaf -recurse -Force ## This one's dangerous!
}

$ONEDRIVESYNC = "$env:USERPROFILE\OneDrive - Aldine Travel"
...
########################################################
## Touch our marker, saying this script has run.
## We do this a little early since the user might fail out of the OneDrive start commands.
$we_ran = New-Item $markerpath -type file
$we_ran.Attributes="Hidden"


########################################################
### Finally, open OneDrive to start the initial sync

# Kill OneDrive if it's open
taskkill.exe /IM "OneDrive.exe"
Start-Sleep 5
taskkill.exe /F /IM "OneDrive.exe"
Start-Sleep 2

# Start Sync (Spawns OneDrive)
